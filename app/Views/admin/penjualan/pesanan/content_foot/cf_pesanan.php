<script>
    $(document).ready(function() {
        ...
        $('#tbl-pesanan').DataTable({
            ...
            "ajax": {
                "url": base_url + '/Admin/Pesanan/load_all_pesanan',
                "type": "POST",
                dataType: 'json',
                "data": function(data) {
    ...
    function kemas_pesanan(id_akun) {

        Swal.fire({
            title: "Apakah anda yakin ingin mengemas pesanan?",
            text: "Anda harus segera mengemas pesanan, dan status transaksi dapat dilihat user",
            icon: 'warning',
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "GET",
                    url: base_url + '/Admin/Pesanan/kemas_pesanan/' + id_akun,
                    beforeSend: function(e) {
                        if (e && e.overrideMimeType) {
                            e.overrideMimeType("application/json;charset=UTF-8");
                        }
                    ...
                    success: function(res) {
                        swal.close();
                        load_pesanan();
                        show_toastr('Pesanan dikemas!', 'success', 'top-right');
                    },
                });
    ...
    function siap_diambil(id_akun) {

        Swal.fire({
            title: "Apakah anda yakin pengemasann pesanan sudah selesai?",
            text: "Pastikan pesanan telah terkemas semua dan pastikan kemasan kuat dan aman.",
            icon: 'warning',
            showDenyButton: true,
            showCancelButton: false,
            confirmButtonText: 'Ya Kemas',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "GET",
                    url: base_url + '/Admin/Pesanan/siap_diambil/' + id_akun,
                    beforeSend: function(e) {
                        if (e && e.overrideMimeType) {
                            e.overrideMimeType("application/json;charset=UTF-8");
                        }
                    ...
                    success: function(res) {
                        swal.close();
                        load_pesanan();
                        show_toastr('Pengemasan pesanan telah selesai dan siap diambil!', 'success', 'top-right');
                    },
                });
            } else if (result.isDenied) {
                swal.close();
